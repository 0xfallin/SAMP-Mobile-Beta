...
void CChatWindow::AddChatMessage(char* szNick, uint32_t dwNickColor, char* szMessage)
{
	...
	strftime(buffffer, 80, "%H:%M:%S", timeinfo);
	char mes[1024];
	sprintf(mes, "%s", szMessage); 
	AddToChatWindowBuffer(CHAT_TYPE_CHAT, mes, szNick, m_dwTextColor, dwNickColor);
}
...
void CChatWindow::AddInfoMessage(const char* szFormat, ...)
{
	...
	strftime(buffffer, 80, "%H:%M:%S", timeinfo);
	char mes[1024];
	sprintf(mes, "%s", tmp_buf);
	AddToChatWindowBuffer(CHAT_TYPE_INFO, mes, nullptr, m_dwInfoColor, 0);
}
...
void CChatWindow::AddDebugMessage(char *szFormat, ...)
{
	...
	char mes[1024];
	sprintf(mes, "%s", tmp_buf);
	AddToChatWindowBuffer(CHAT_TYPE_DEBUG, mes, nullptr, m_dwDebugColor, 0);
}
...
void CChatWindow::AddClientMessage(uint32_t dwColor, char* szStr)
{
	...
	char mes[1024];
	sprintf(mes, "%s", szStr);
	AddToChatWindowBuffer(CHAT_TYPE_INFO, mes, nullptr, dwColor, 0);
}

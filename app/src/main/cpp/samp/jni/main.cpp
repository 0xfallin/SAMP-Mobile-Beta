...
void InitNetwork()
{
	if(!bNetworkInited && pSettings->Get().iFontOutline)
	{
		...
		#else
			pNetGame = new CNetGame(encryptedAddress.decrypt(), port, pSettings->Get().szNickName, "koshi"); //pw server
		#endif
...
Java_ro.samp.mobile_SampLauncher_initSAMP(JNIEnv* env, jobject thiz, jstring storagePath) {
...
JNIEXPORT void JNICALL
Java_ro.samp.mobile_SampLauncher_startGameLoop(JNIEnv* env, jobject thiz) {
    while (!bQuitGame) {
        ProcessSAMPGraphic();
...
JNIEXPORT void JNICALL
Java_ro.samp.mobile_SampLauncher_quitGame(JNIEnv* env, jobject thiz) {
    QuitGame();
}

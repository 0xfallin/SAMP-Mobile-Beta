...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
    ...
    if(pBKeys) pBKeys->Show(true);
    
    if(pChatWindow)
	{
		pChatWindow->AddDebugMessage("DEVELOPER LUFZY");
		pChatWindow->AddDebugMessage("{FFFFFF}SA-MP {B9C9BF}" SAMP_VERSION "{FFFFFF} Started");
		pChatWindow->AddDebugMessage("{00FF00}SAMP Android -zy {00FF00}1.3");
	}
}
...
void CNetGame::Process()
{
    ...
    {
        if(pChatWindow) pChatWindow->AddDebugMessage("Connecting to the server...");
